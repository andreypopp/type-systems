...
type expr =
  ...
  | E_lit of lit
  | E_tclass_method of expr * string
[@@deriving sexp_of]
...
and bound = B_class of name * ty list

and cty = bound list * ty
...
let rec layout_expr' ~names =
  ...
  | E_tclass_method (e, name) -> layout_expr' ~names e ^^ dot ^^ string name
...
and layout_bound' ~names =
  let open PPrint in
  let sep = comma ^^ blank 1 in
  function
  | B_class (name, vs) ->
    string name ^^ brackets (separate sep (List.map vs ~f:(layout_ty' ~names)))

and layout_cty' ~names =
  let open PPrint in
  function
  | [], ty -> layout_ty' ~names ty
  | bounds, ty ->
    let sep = comma ^^ blank 1 in
    group (separate sep (List.map bounds ~f:(layout_bound' ~names)))
    ^^ string " =>"
    ^^ break 1
    ^^ group (layout_ty' ~names ty)
...
module Bound = struct
  ...
  let to_ty (B_class (name, tys)) = Ty_app (Ty_const name, tys)

  include (
    Showable (struct
      type t = bound

      let layout b = layout_bound' ~names:(Names.make ()) b
    end) :
      SHOWABLE with type t := t)
...
module Cty = struct
  ...
  include (
    Showable (struct
      ...
      let layout cty = layout_cty' ~names:(Names.make ()) cty
    end) :
      SHOWABLE with type t := t)
